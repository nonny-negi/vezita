...
exports.udpateAvailability = catchAsyncErrors( async(req,res) =>{
        ...
        if(service.serviceType==='fixed_price' && type==='add' && req.body.fixedPriceAvailability){
            for(let i=0;i<req.body.fixedPriceAvailability.length;i++){
                availability.fixedPriceAvailability.push(req.body.fixedPriceAvailability[i])
            }
            await availability.save()
        }
        if(service.serviceType==='hourly_price' && type==='add' && req.body.hourlyAvailability){
        ...
        if(service.serviceType==='fixed_price' && type==='remove' && req.body.fixedPriceAvailabilityRemoveList){
            for(let i=0;i<req.body.fixedPriceAvailabilityRemoveList.length;i++){
                availability.fixedPriceAvailability.pull({_id:req.body.fixedPriceAvailabilityRemoveList[i]})
            }
            await availability.save()
        }

        //for remove availability
        if(service.serviceType==='hourly_price' && type==='remove' && req.body.hourlyAvailabilityRemoveList){
            for(let i=0;i<req.body.hourlyAvailabilityRemoveList.length;i++){
                availability.hourlyAvailability.pull({_id:req.body.hourlyAvailabilityRemoveList[i]})
            }
            await availability.save()
        }

        if(service.serviceType==='fixed_price' && type==='changeAvailability' && req.body.fixedPriceChangeAvailability){
            for(let i=0;i<req.body.fixedPriceChangeAvailability.length;i++){
                let update = await ServiceAvailability.findOneAndUpdate(
                    {"fixedPriceAvailability._id":req.body.fixedPriceChangeAvailability[i]._id},
                    {$set:{"fixedPriceAvailability.$.isAvailable":req.body.fixedPriceChangeAvailability[i].isAvailable}},
                    {new:true}
                )                
        ...
        if(service.serviceType==='hourly_price' && type==='changeAvailability' && req.body.hourlyChangeAvailability){
            for(let i=0;i<req.body.hourlyChangeAvailability.length;i++){
                let update = await ServiceAvailability.findOneAndUpdate(
                    {"hourlyAvailability._id":req.body.hourlyChangeAvailability[i]._id},
                    {$set:{"hourlyAvailability.$.isAvailable":req.body.hourlyChangeAvailability[i].isAvailable}},
                    {new:true}
                )                
            }
            let updatedAvailability = await ServiceAvailability.findById(availabilityId)
